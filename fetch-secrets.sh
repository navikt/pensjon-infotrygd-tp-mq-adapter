...
fetch_kubernetes_secrets "AzureAD" "dev-fss" "pensjonsamhandling" "azure-pensjon-infotrygd-tp-mq-adapter-$env" "strict" \
    "AZURE_APP_CLIENT_ID" \
...
echo "SRVTPMQ_USERNAME='$(vault kv get -field username -mount=serviceuser dev/srvtpmq)'" >> "${envfile}"
echo "SRVTPMQ_PASSWORD='$(vault kv get -field password -mount=serviceuser dev/srvtpmq)'" >> "${envfile}"
